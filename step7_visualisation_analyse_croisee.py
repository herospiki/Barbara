...
# Configuration
input_csv = "final/pontes_meteo.csv"
output_html = "final/analyse_croisee.html"
...
def charger_donnees():
    ...
    df['Effectif_poules'] = df['Effectif_poules'].fillna(0)
...
def calculer_kpis(df):
    ...
    df_active = df[df['Effectif_poules'] > 0].copy()
    
    df_active['Taux_Ponte_%'] = (df_active['Nombre_pontes'] / df_active['Effectif_poules'] * 100).round(1)
...
def generer_visualisations(df):
    ...
    df_jour = df[df['Effectif_poules'] > 0].copy()
    df_jour['Taux_Ponte_%'] = (df_jour['Nombre_pontes'] / df_jour['Effectif_poules'] * 100).round(1)
    ...
    fig0 = px.scatter(df_jour, x='T°C (12h-15h)', y='Taux_Ponte_%', 
                      ...
                      hover_data=['Date', 'Effectif_poules', 'Nombre_pontes'],
    ...
    hebdo = df.groupby('Semaine').agg({
        'Nombre_pontes': 'sum',
        'Effectif_poules': 'mean', # Effectif moyen sur la semaine
        'T°C (12h-15h)': 'mean'
    }).reset_index()
    ...
    hebdo['Taux_Ponte_Hebdo_%'] = (hebdo['Nombre_pontes'] / (hebdo['Effectif_poules'] * 7) * 100).round(1)
